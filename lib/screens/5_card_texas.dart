...
class CardTexasPage extends StatefulWidget {
  const CardTexasPage({
    super.key,
...
class _CardTexasPageState extends State<CardTexasPage> {
  ...
  Future<void> _showRemovePlayersDialog() async {
    ...
    final removedUserIds = selectedIndexes
  ...
  String _buildSessionReportText() {
    final lines = <String>[
      '5 МОДНЫ ТЕХАС - ТОГЛОЛТЫН ТАЙЛАН',
    ...
    try {
      await SharePlus.instance.share(
        ShareParams(
          ...
          subject: '5 модны Техас - тоглолтын тайлан',
        ),
      );
    } catch (_) {
      if (!mounted) return;
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text('5 МОДНЫ ТЕХАС - ТОГЛОЛТЫН ТАЙЛАН'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: UnifiedGameAppBar(
        title: const Text('5 модны Техас'),
